...
class Calendar_App:
    ...
    def adding_data(self, current_data):

            fh = open("calendar.json","r")
    ...
    def file_handling(self, current_data=None):

        if os.path.exists("calendar.json"):
            if current_data is not None:
                self.adding_data(current_data)
            else:
                fh = open("calendar.json","r")
                data = json.load(fh)["data"]
                data.append({"Date":self.values[0],"Event":self.values[1]})

                with open("calendar.json","w") as fh:
                    json.dump({"data":data}, fh)
        else:
            fh = open("calendar.json","w")
            data = {"data":[{"Date":self.values[0],"Event":self.values[1]}]}
            json.dump(data, fh)
    ...
    def display_calendar(self):
        ...
        for idx, data in enumerate(data):
            ...
            self.add_link(root, main_text, date, event)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        action = input("Would you like to do? To create an event enter create and to check the calendar enter check. ")
